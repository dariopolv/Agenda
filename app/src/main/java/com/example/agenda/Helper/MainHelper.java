...
import com.example.agenda.Class.DBController;
...
public class MainHelper {
    ...
    private Boolean setModifyMode;
    private DBController dbController;
    private ListHelper listHelper;
    private ArrayList<String> timeList, workList, idList, noteList;
    private Date date;
        ...
        if (snapshot.getValue() != null) {
            ...
            while (count != key.length() - 2) {
                if (index > 0) {
                    ...
                    String note = value.substring(value.indexOf("/") +1 , value.length());
                    String timeVal = value.substring(value.indexOf("/") - 5, value.indexOf("/"));
                    String workVal = value.substring(0, value.indexOf("/") - 6);
                    loadList(workVal, timeVal, id, note);
                } else {
                    count = key.length() - 2;
                }
    ...
    public void loadList(String workValue, String timeValue, String id, String note) {
        if (check(idList, id)) {
            sortList(timeList, timeValue, workList, workValue, idList, id, noteList, note);
        }
        timeAdapter = new ArrayAdapter(context, android.R.layout.simple_list_item_1, timeList);
        workAdapter = new ArrayAdapter(context, android.R.layout.simple_list_item_1, workList);
        workListView.setAdapter(workAdapter);
    ...
    public void sortList(ArrayList<String> arrayListTime, String valueTime, ArrayList<String> arrayListWork,
                         String valueWork, ArrayList<String> arrayListId, String valueId, ArrayList<String> noteList, String note) {
        boolean check = true;
    ...
    public void goToModifyWork(int index, String currentDay, Intent intent) {
        intent.putExtra("index", index);
        intent.putStringArrayListExtra("worklist", getWorkList());
        intent.putStringArrayListExtra("timelist", getTimeList());
        intent.putStringArrayListExtra("idlist", getIdList());
        intent.putStringArrayListExtra("notelist", getNoteList());
    ...
    public Boolean getSetModifyMode() {
        return setModifyMode;
    }

    public void setSetModifyMode(Boolean setModifyMode) {
